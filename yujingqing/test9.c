...
#define NAME_MAX 30//地域名的最大长度

#define WEIGHT_MAX 30000//最大权值（以公里为单位）
typedef struct AA

{
	...
	int weight;//边的权值，也就是两区域的距离

	struct AA *next;//邻接表结点的指针域
...

{
	...
	for(i=1;i<=length;i++)
		...
		if(strcmp(str,metro[i].name)==0)//strcmp函数用于比较两个字符串的大小，这里是用来判断s字符串类型的数组.地域名是否没有

			break;

	if(i>length)//判断此时的metro[i]有没有出表，不在表内。
...

{
	...
	Node *s=(Node *)malloc(sizeof(Node));//为节点指针动态分配内存
...

{
	...
	for(i=1;i<n+1;i++)//循环次数，直至结点信息补充完全

	{
		...
		strcpy(metro[i].name,name);//将name中的地址空间复制到metro[i]的name中
	...
	printf("请输入各条边的信息（格式：辖区名 称辖区名称 直接距离）\n");

	for(i=1;i<=edge;i++)//补充距离信息

	{

		printf("请输入第%d条边:",i);

		char str1[NAME_MAX],str2[NAME_MAX];//定义数组str1[30] str2[30]
		...
		if(w1==0||w2==0||weight<=0)//=0时是存储的内容出表了或俩区域距离超过限定值
