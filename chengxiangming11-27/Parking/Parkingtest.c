        ...
        switch(i){
            case 1:
                if(ListLength(L)==7){//停车场顺序表长度满7
                    if(QueueLength(q1)==3)  cout<<"目前停车场已满"<<endl<<endl;
                    else if(QueueLength(q1)!=3){
                        cout<<"     目前停车场已满，请等待"<<endl<<endl;
                        Gettt1(e);//车牌
                         e1=e.num;
                        EnQueue(q1,e1);//进队等
                    }
                    ...
                    if(ListLength(L)==0)  cout<<"欢迎您"<<endl;
                    else{
                        cout<<"当前已被使用的车位有：";
                ...
                break;
            case 2:
                if(QueueLength(q2)==3)  cout<<"目前等待结算的车辆较多"<<endl;
                else{
                    if(ListEmpty(L))  cout<<"空位"<<endl;
                    else{
                        ...
                        cin>>i1.num;//车牌
                         if(ListDelete(L,LocateElem(L,i1),e)){//查找并删除，e是被删的车
                            ...
                            if(!QueueEmpty(q1)){//进队列非空
                                DeQueue(q1,e1);//出队（车牌
                                Getn1(e,e1);//车牌获取
                                cout<<"车牌为"<<e.num<<"的车辆现可以入库！请确定入库时间："<<endl;
                                Gett(e);//进场时间
            ...
            case 3:
                if(QueueEmpty(q2))  cout<<"当前暂无等待结算离开的车辆"<<endl;
                    ...
                    DeQueue(q2,e2);//出队（车牌
                    cout<<"车牌为"<<e2<<"的车辆现进行费用结算！请输入当前时间：";
                    DeQueue(q3,e3);//出队（时间
                    Gettt(e3);//算钱
