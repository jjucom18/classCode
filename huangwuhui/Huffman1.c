...
void selecte(HufNode *huf_tree, unsigned int n, int *s1, int *s2)
{
	// 找最小
	unsigned int i;
...
void CreateTree(HufNode *huf_tree, unsigned int char_kinds, unsigned int node_num)
{
	...
	{ 
		selecte(huf_tree, i, &s1, &s2);		// 选择最小的两个结点
...
int main()
{
	while(1)
	{
		...
		{
			printf("请输入需要压缩或解压的文件名称（例：read.txt）: ");
			fflush(stdin);		// 清空标准输入流，防止干扰gets函数读取文件名
			fgets(ifname,20,stdin);
			if(strlen(ifname)!=sizeof(ifname)-1)
				ifname[strlen(ifname)-1]='\0';
			printf("请输入输出文件名称: ");
			fflush(stdin);
			fgets(ofname,20,stdin );
			if(strlen(ofname)!=sizeof(ofname)-1)
