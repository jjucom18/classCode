...
#define M 2000            //假设两顶点之间没有路径,用5000来表示
...

{
	...
	for(j=1;j<=10;j++)
      ...
      //初始化顶点信息
		...
		strcpy(G->vexs[1].introduction ,"学生出入最多的大门");

		strcpy(G->vexs[2].name ,"大礼堂");

		strcpy(G->vexs[2].introduction ,"日常庆典活动，社团演出");
        ...
        strcpy(G->vexs[5].introduction ,"是九院的标志建筑，也是江西省最大的教学楼");

        strcpy(G->vexs[6].name ,"南区操场");

		strcpy(G->vexs[6].introduction ,"每年各个学院大一新生都聚集这里召开运动会");

        strcpy(G->vexs[7].name ,"三食堂");

        strcpy(G->vexs[7].introduction ,"三食堂的饭菜便宜，人流量最多的食堂");

        strcpy(G->vexs[8].name ,"男生15栋");

        strcpy(G->vexs[8].introduction ,"电子学院男生宿舍");

        strcpy(G->vexs[9].name ,"莲池");

        strcpy(G->vexs[9].introduction ,"九江学院环境最好的地方");

        strcpy(G->vexs[10].name ,"九江学院西门");

		strcpy(G->vexs[10].introduction ,"学生出入大门，也是升国旗的地方");
...
if(G->arc[b][j]<2000&&G->vexs[j].park==0){
                    ...
                    RudeGraph(G,j,w,k,sum);    //递归对所有路径进行深度搜索
